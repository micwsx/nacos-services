...
public class AuthFilter implements GlobalFilter {
    ...
    private static final String OAUTH_ACCESS_TOKEN_URL = "/oauth/accesstoken";
    private static final String OAUTH_LOGOUT_URL = "/oauth/logout";
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        Map<String, String> validateResultMap = null;
        ...
        try {
            ...
            if (StringUtils.isEmpty(token)) {
                //是登录接口则放行，否则返回异常
                if (path.contains(OAUTH_ACCESS_TOKEN_URL)) {
                    return chain.filter(exchange);
                }else{
                    throw new PermissionException("身份未认证[token为空]");
                }
            }
            ...
            //4- 处理刷新token请求
            if (path.contains(OAUTH_LOGOUT_URL) && HttpMethod.DELETE.equals(method)) {
                //放行去掉登出接口（在刷新前校验userId和token是否匹配）
                return chain.filter(exchange);
            }

            //5- 携带token请求其他业务接口
            validateResultMap = JwtUtil.validateTokenAndUser(token, userId);
            if (validateResultMap == null || validateResultMap.isEmpty()) {
                throw new PermissionException("token 已经失效");
            }
        } catch (PermissionException e) {
            Mono<Void> completion= Mono.error(e);
            return completion;
        }
